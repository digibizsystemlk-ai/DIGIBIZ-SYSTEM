        ...
        if (window.AggregateUtils) {
            try {
                [dailyAgg, monthlyAgg] = await Promise.all([
                    window.AggregateUtils.getDailyAggregate(window.db, bid),
                    window.AggregateUtils.getMonthlyAggregate(window.db, bid)
                ]);
            } catch (eAgg) {
                console.warn('[DashboardCore] Distributor aggregates notice:', eAgg);
            }
        }

        if (dailyAgg && typeof dailyAgg.sales_amount === 'number' && monthlyAgg && typeof monthlyAgg.sales_amount === 'number') {
    ...
    async getRetailMetrics(context) {
        ...
        if (window.AggregateUtils) {
            try {
                ...
                console.warn('[DashboardCore] Retail aggregates notice:', eAgg);
            }
        }

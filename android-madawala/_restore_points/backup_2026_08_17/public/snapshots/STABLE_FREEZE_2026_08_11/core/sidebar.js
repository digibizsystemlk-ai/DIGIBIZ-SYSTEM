...
class Sidebar {
    ...
    getMenus() {
        ...
        if (isQuickBilling) {
            ...
            return [
                ...
                { icon: "📦", name: "Manage Products & Prices", link: "/modules/quick_billing/app.html?view=products" },
                { icon: "👥", name: "Customers Directory", link: "/modules/quick_billing/app.html?view=customers" },
                ...
                <div>
                    ...
                    <div class="nav-links" id="sidebarNavLinks">
                        ...
                        ${(!isQuickBilling && this.businessType !== 'attendance_payroll' && canViewSettings && settingsItems.length > 0) ? `<div class="menu-dropdown ${settingsActive ? 'open' : ''}" id="settingsDropdown">
                            ...
                            <div class="menu-dropdown-items">
                                ${settingsItems.map((item) => `<a href="${item.link}" target="${SIDEBAR_NAV_LINK_TARGET}" rel="${SIDEBAR_NAV_LINK_REL}" class="menu-item ${this.isMenuActive(item.link, pathname) ? 'active' : ''}"><span class="menu-icon">${item.icon}</span><span>${window.i18n(item.name)}</span></a>`).join('')}
                            </div>
                        </div>` : ''}
                        ...
                        <a href="/admin/super-dashboard.html" target="${SIDEBAR_NAV_LINK_TARGET}" rel="${SIDEBAR_NAV_LINK_REL}" class="menu-item ${this.isMenuActive('/admin/super-dashboard.html', pathname) ? 'active' : ''}"><span class="menu-icon">👑</span><span>${window.i18n('Super Admin')}</span></a>
                        <a href="/admin/business-management.html" target="${SIDEBAR_NAV_LINK_TARGET}" rel="${SIDEBAR_NAV_LINK_REL}" class="menu-item ${this.isMenuActive('/admin/business-management.html', pathname) ? 'active' : ''}"><span class="menu-icon">💼</span><span>${window.i18n('Business Management')}</span></a>
                        <a href="/admin/client-version-control.html" target="${SIDEBAR_NAV_LINK_TARGET}" rel="${SIDEBAR_NAV_LINK_REL}" class="menu-item ${this.isMenuActive('/admin/client-version-control.html', pathname) ? 'active' : ''}"><span class="menu-icon">🛡️</span><span>${window.i18n('Client Version Control')}</span></a>` : ''}
                </div>
    ...
    updateUserInfo() {
        ...
        if (subEl) {
            ...
            const plan = String((this.subscriptionState && this.subscriptionState.plan) || bData.plan || bData.subscriptionPlan || bData.status || 'TRIAL').toUpperCase();
            const rawStatus = String((this.subscriptionState && this.subscriptionState.status) || bData.status || bData.subscriptionStatus || 'TRIAL').toUpperCase();

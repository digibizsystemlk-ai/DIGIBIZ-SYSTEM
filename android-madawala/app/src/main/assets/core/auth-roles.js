...
async function getUserRole(userId, businessId = null) {
    ...
    try {
        ...
        const em = String((window.auth && window.auth.currentUser && window.auth.currentUser.email) || (firebase.auth && firebase.auth().currentUser && firebase.auth().currentUser.email) || '').trim().toLowerCase();

        // 0. SUPER ADMIN FAST PATH
        const isSuperAdminEmail = (em === 'biz.sirimal@gmail.com' || em === '2biz.sirimal@gmail.com' || em === 'scrap@chinthaka.com' || userId === 'oDhSDYHQ2dV1DP33koysmZAqaY13');
        const isImpersonating = localStorage.getItem('digibiz_impersonate_active') === 'true';
        if (isSuperAdminEmail && !isImpersonating) {
            return { role: 'SUPER_ADMIN', businessId: (typeof localStorage !== 'undefined' ? localStorage.getItem('currentBusinessId') : null) };
        }

        // 1. DEDICATED CLIENT RESOLUTION
        const authUser = (window.auth && window.auth.currentUser) || (firebase.auth && firebase.auth().currentUser);
        const dedicated = (window.DigibizClientMap && typeof window.DigibizClientMap.resolveDedicatedClient === 'function')
            ? window.DigibizClientMap.resolveDedicatedClient(authUser)
            : null;
        if (dedicated) {
            return { role: 'BUSINESS_OWNER', businessId: dedicated.businessId };
        }

        // 2. GLOBAL STAFF REGISTRY PRIORITY
        if (em && !isSuperAdminEmail) {
            try {
                const regDoc = await db.collection('staff_registry').doc(em).get();
                if (regDoc.exists && regDoc.data().businessId) {
                    const regData = regDoc.data();
                    return { role: String(regData.role || 'CASHIER').toUpperCase(), businessId: regData.businessId };
                }
            } catch (eReg) { /* ignore */ }
        }

        // 3. BUSINESS CONTEXT PRIORITY (Security first)
        if (resolvedBusinessId) {
            ...
            if (bizDoc.exists) {
                ...
                const authUserObj = firebase.auth().currentUser;
                const authEmail = String(authUserObj && authUserObj.email || '').toLowerCase();
        ...
        return { role: globalRole || 'BUSINESS_OWNER', businessId: resolvedBusinessId || null };
    } catch (error) {
...
// Role Discovery & Business-type specific role standards
function getStandardRolesForBusinessType(businessType) {
    ...
    if (bt === 'distributor') {
        return [
            ...
            { id: 'DELIVERY_MANAGER', label: 'Delivery Manager' },
            { id: 'ACCOUNTANT', label: 'Accountant' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator' }
        ];
    } else if (bt === 'retail' || bt === 'supermarket' || bt === 'hardware' || bt === 'pharmacy') {
        return [
            { id: 'CASHIER', label: 'Cashier (කැෂියර් / Billing)' },
            { id: 'MANAGER', label: 'Manager (කළමනාකරු)' },
            { id: 'STORE_KEEPER', label: 'Store Keeper (තොග පාලක)' },
            { id: 'ACCOUNTANT', label: 'Accountant (ගණකාධිකාරී)' },
            { id: 'STAFF', label: 'Sales Staff / Clerk (සේවක)' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator (හිමිකරු)' }
        ];
    } else if (bt === 'tire_centre' || bt === 'garage') {
        return [
            { id: 'CASHIER', label: 'Cashier (කැෂියර්)' },
            { id: 'STORE_KEEPER', label: 'Spare Parts Store Keeper' },
            { id: 'STAFF', label: 'Technician / Mechanic' },
            { id: 'MANAGER', label: 'Service Manager' },
            { id: 'ACCOUNTANT', label: 'Accountant' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator' }
        ];
    } else if (bt === 'bakery') {
        return [
            { id: 'MANAGER', label: 'Bakery Supervisor / Head Baker (ප්‍රධාන බේකරි අධීක්ෂක)' },
            { id: 'STORE_KEEPER', label: 'Ingredients & Bread/Pastry Keeper (අමුද්‍රව්‍ය හා නිමි තොග පාලක)' },
            { id: 'CASHIER', label: 'Sales & Billing Cashier (අලෙවි කැෂියර්)' },
            { id: 'STAFF', label: 'Kitchen & Baking Staff (බේකරි සේවක)' },
            { id: 'ACCOUNTANT', label: 'Accountant (ගණකාධිකාරී)' },
            { id: 'BUSINESS_OWNER', label: 'Bakery Owner / Administrator (හිමිකරු)' }
        ];
    } else if (bt === 'coconut' || bt === 'manufacturer') {
        return [
            { id: 'MANAGER', label: 'Production / Factory Supervisor' },
            { id: 'STORE_KEEPER', label: 'Raw Material & Finished Goods Keeper' },
            { id: 'CASHIER', label: 'Sales / Billing Cashier' },
            { id: 'STAFF', label: 'Procurement / Factory Staff' },
            { id: 'ACCOUNTANT', label: 'Accountant' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator' }
        ];
    } else if (bt === 'attendance_payroll') {
        return [
            { id: 'HR_MANAGER', label: 'HR Manager (මානව සම්පත්)' },
            { id: 'MANAGER', label: 'Shift Supervisor (අධීක්ෂක)' },
            { id: 'ACCOUNTANT', label: 'Payroll Accountant' },
            { id: 'STAFF', label: 'General Staff (සාමාන්‍ය සේවක)' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator' }
        ];
    } else if (bt === 'quick_billing' || bt === 'easy_bill' || bt === 'quick_bill') {
        return [
            { id: 'CASHIER', label: 'Billing Cashier (බිල්පත් කැෂියර්)' },
            { id: 'MANAGER', label: 'Store Manager (කළමනාකරු)' },
            { id: 'STAFF', label: 'Assistant / Staff (සහායක)' },
            { id: 'BUSINESS_OWNER', label: 'Owner / Administrator (හිමිකරු)' }
        ];
    } else {
        return [
...
window.getStandardRolesForBusinessType = getStandardRolesForBusinessType;

/**
 * Dynamic Role Discovery Utility
 * Scans the staff collection of a business to find all unique roles in use.
 */
window.getBusinessStaffRoles = async function(businessId, businessType = null) {
    ...
    try {
        ...
        // 3. Populate default standard roles for this business type so the matrix is pre-populated
        const stdRoles = getStandardRolesForBusinessType(businessType);
        stdRoles.forEach(r => {
            const band = window.DigibizDistributorPermissions.roleBand(r.id);
            roles.add(band);
        });
        ...
        return ['OWNER', 'CASHIER', 'MANAGER', 'STORE_KEEPER', 'ACCOUNTANT'];
    }
};

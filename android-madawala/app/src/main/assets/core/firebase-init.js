const liveConfig = {
    apiKey: "AIzaSyDHGCzs-dfkHUydC0aHCejFDEcg5QZM9cY",
    authDomain: "digibiz-pro.firebaseapp.com",
    projectId: "digibiz-pro",
    storageBucket: "digibiz-pro.firebasestorage.app",
    messagingSenderId: "372856583330",
    appId: "1:372856583330:web:48f7d37642c2b2b783d3fb",
    databaseURL: "https://digibiz-pro-default-rtdb.firebaseio.com"
};
window.DIGIBIZ_LIVE_CONFIG = liveConfig;

const firebaseConfig = liveConfig;
...
// Unified Domain Enforcer: Keeps all client portals seamlessly running under the unified hosting origin
(function enforceCanonicalDomain() {
    // External web.app redirects disabled for unified client folder architecture
})();

window.digibizLogout = function () {
    try {
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
        firebase.auth().signOut().finally(function () {
            window.location.href = 'https://digibiz-pro.web.app/';
        });
    } else {
        window.location.href = 'https://digibiz-pro.web.app/';
    }
};
window.signOutUser = window.digibizLogout;
window.executeSignOut = window.digibizLogout;
...
(function() {
    ...
    const isRetailContext = window.location.pathname.includes('/retail/') ||
                           ...
                           window.location.pathname.includes('/admin/core/') ||
...
(function () {
    ...
    const MW_TRADING_BUSINESS_ID = '13Gu7XIBcBZJFS61Ub42sUV7ger1';
    ...
    window.auth.onAuthStateChanged(function (user) {
        ...
        } else if (email === 'sathityre05@gmail.com' || email.includes('sathityre')) {
            try {
                const SATHI_BIZ_ID = 'NdhXszkWHAhf3pNBtSQGKpHmwEr1';
                localStorage.setItem('currentBusinessId', SATHI_BIZ_ID);
                sessionStorage.setItem('currentBusinessId', SATHI_BIZ_ID);
                localStorage.setItem('businessId', SATHI_BIZ_ID);
                localStorage.setItem('currentBusinessType', 'tire_centre');
                sessionStorage.setItem('currentBusinessType', 'tire_centre');
                localStorage.setItem('currentUserRole', 'BUSINESS_OWNER');
                sessionStorage.setItem('currentUserRole', 'BUSINESS_OWNER');
                localStorage.setItem('currentBusinessNavRole', 'BUSINESS_OWNER');
                sessionStorage.setItem('currentBusinessNavRole', 'BUSINESS_OWNER');
                localStorage.setItem('currentBusinessName', 'SATHI TYRE HOUSE & MOTORES');
                sessionStorage.setItem('currentBusinessName', 'SATHI TYRE HOUSE & MOTORES');
            } catch (e) { /* ignore */ }
        } else if (email === 'madawalateashop80@gmail.com' || email.includes('madawalateashop') || email.includes('darshana')) {
            try {
                const DM_BIZ_ID = 'Qxl4JRGfBZTZOFMoQYhocFR2pfp2';
                localStorage.setItem('currentBusinessId', DM_BIZ_ID);
                sessionStorage.setItem('currentBusinessId', DM_BIZ_ID);
                localStorage.setItem('businessId', DM_BIZ_ID);

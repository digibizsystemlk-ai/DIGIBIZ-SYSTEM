...
exports.sendNewUserNotification = onCall(
    ...
    async (request) => {
        ...
        const safeOwnerName = escapeHtml(ownerName || 'N/A');
        const safeBusinessName = escapeHtml(businessName || 'N/A');
        const safeFormattedType = escapeHtml(formattedType);
        const safeUserEmail = escapeHtml(userEmail);
        const safePhone = escapeHtml(phone || 'Not provided');
        const safeDescription = escapeHtml(description || 'None');
    ...
    <div style="padding: 24px 28px;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 13.5px;">
        <tr style="border-bottom: 1px solid #f1f5f9;">
          ...
          <td style="padding: 10px 4px; font-weight: 800; color: #0f172a;">${safeOwnerName}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 10px 4px; font-weight: 700; color: #64748b;">🏢 Business Name:</td>
          <td style="padding: 10px 4px; font-weight: 800; color: #059669;">${safeBusinessName}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 10px 4px; font-weight: 700; color: #64748b;">🏷️ Business Type:</td>
          <td style="padding: 10px 4px; font-weight: 700; color: #d97706;">${safeFormattedType}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 10px 4px; font-weight: 700; color: #64748b;">✉️ Email Address:</td>
          <td style="padding: 10px 4px; font-weight: 700; color: #2563eb;"><a href="mailto:${safeUserEmail}" style="color:#2563eb; text-decoration:none;">${safeUserEmail}</a></td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 10px 4px; font-weight: 700; color: #64748b;">📞 Phone Number:</td>
          <td style="padding: 10px 4px; font-weight: 700; color: #0f172a;">${safePhone}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 10px 4px; font-weight: 700; color: #64748b;">📝 Notes / Info:</td>
          <td style="padding: 10px 4px; color: #334155;">${safeDescription}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
        ...
        <tr>
          ...
          <td style="padding: 10px 4px; font-family: monospace; font-size: 12px; color: #64748b;">${safeUserId}</td>
        </tr>
      </table>
...
exports.sendMfaEmailOtp = onCall(
    ...
    async (request) => {
        ...
        // Rate limiting: 45s cooldown per email
        try {
            const existingOtpDoc = await db.collection("mfa_otps").doc(email).get();
            if (existingOtpDoc.exists) {
                const existingData = existingOtpDoc.data() || {};
                const lastCreated = existingData.createdAt ? (existingData.createdAt.toMillis ? existingData.createdAt.toMillis() : Number(existingData.createdAt)) : 0;
                if (lastCreated && (Date.now() - lastCreated < 45000)) {
                    throw new HttpsError("resource-exhausted", "Please wait a moment before requesting another verification code.");
                }
            }
        } catch (eRate) {
            if (eRate instanceof HttpsError) throw eRate;
        }
        ...
        try {
            ...
            throw new HttpsError("internal", "Failed to dispatch verification code. Please try again later.");
        }
    }
);
...
exports.verifyMfaEmailOtp = onCall(
    ...
    async (request) => {
        ...
        try {
            ...
            if (String(data.otp).trim() !== code) {
                await db.collection("mfa_otps").doc(email).update({ attempts: attempts }).catch(() => {});
                const rem = Math.max(0, 5 - attempts);
                return { success: false, message: `Incorrect OTP code. (${rem} attempts remaining)` };
            }
...
exports.deleteUserAccountByEmail = onCall(
    ...
    async (request) => {
        ...
        const requesterEmail = String((auth && auth.token && auth.token.email) || "").trim().toLowerCase();
        const allowedAdmins = ["biz.sirimal@gmail.com", "2biz.sirimal@gmail.com", "scrap@chinthaka.com", "chinthaka@gmail.com"];
        let isSuperAdmin = allowedAdmins.includes(requesterEmail) || (auth && auth.uid === "oDhSDYHQ2dV1DP33koysmZAqaY13") || (auth && SUPER_ADMIN_UIDS.includes(auth.uid));

        if (!isSuperAdmin && auth && auth.uid) {
            try {
                const uDoc = await db.collection('users').doc(auth.uid).get();
                if (uDoc.exists) {
                    const uData = uDoc.data() || {};
                    if (uData.role === 'SUPER_ADMIN' || uData.globalRole === 'SUPER_ADMIN' || uData.isSuperAdmin === true) {
                        isSuperAdmin = true;
                    }
                }
            } catch (eRole) {}
...
/**
 * Self-healing Stale Auth Wiping Tool for Registration
 * Safely removes orphaned Auth records when a user attempts registration if no business profile exists.
 */
exports.wipeStaleAuthOnRegister = onCall(
    {
        timeoutSeconds: 60,
        memory: "256MiB",
    },
    async (request) => {
        ...
        // Safety verification: Do not wipe if an active business profile exists
        const [bizSnap1, bizSnap2] = await Promise.all([
            db.collection('businesses').where('ownerEmail', '==', targetEmail).limit(1).get().catch(() => ({ empty: true })),
            db.collection('businesses').where('email', '==', targetEmail).limit(1).get().catch(() => ({ empty: true }))
        ]);

        if (!bizSnap1.empty || !bizSnap2.empty) {
            throw new HttpsError("already-exists", "An active business profile already exists for this email address.");
        }
        ...
        try {
            ...
            console.warn('[WipeStale] Auth delete notice:', eAuth.message);
        }
        ...
        try {
            ...
            console.warn('[WipeStale] User delete notice:', eUser.message);
        }
...
/**
 * Super Admin Inactive Accounts Purge Tool
 * Completely deletes inactive users and their orphaned businesses from Auth and Firestore.
 * This frees up the email so it can be re-registered immediately.
 */
exports.adminPurgeInactiveUserAccounts = onCall(
    {
        timeoutSeconds: 120,
        memory: "512MiB",
    },
    async (request) => {
        ...
        const targets = Array.isArray(data.targets)
            ? data.targets
            : (data.targetEmail ? [{ email: data.targetEmail, uid: data.targetUid, businessId: data.businessId }] : []);

        if (!targets.length) {
            throw new HttpsError("invalid-argument", "At least one target user is required.");
        }

        const results = [];
        for (const item of targets) {
            const targetEmail = String(item.email || "").trim().toLowerCase();
            let targetUid = item.uid ? String(item.uid).trim() : null;
            const businessId = item.businessId ? String(item.businessId).trim() : null;
            ...
            try {
                if (!targetUid && targetEmail) {
                    const uRec = await admin.auth().getUserByEmail(targetEmail).catch(() => null);
                    if (uRec) targetUid = uRec.uid;
                }
                if (targetUid) {
                    await admin.auth().deleteUser(targetUid);
                    authDeleted = true;
                ...
                console.warn('[adminPurgeInactive] Auth delete warn:', eAuth.message);
            }
            ...
            try {
                ...
                console.warn('[adminPurgeInactive] User doc delete warn:', eUser.message);
            }

            // 3. Delete from Firestore businesses & settings collection
            try {
                if (businessId) {
                    await db.collection('businesses').doc(businessId).delete();
                    await db.collection('settings').doc(businessId).delete().catch(() => {});
                    bizDocsDeleted++;
                }
                if (targetUid && targetUid !== businessId) {
                    const bDoc = await db.collection('businesses').doc(targetUid).get();
                    if (bDoc.exists) {
                        await db.collection('businesses').doc(targetUid).delete();
                        await db.collection('settings').doc(targetUid).delete().catch(() => {});
                        bizDocsDeleted++;
                    }
                }
                if (targetEmail) {
                    ...
                    for (const d of bSnap.docs) {
                        await db.collection('businesses').doc(d.id).delete();
                        await db.collection('settings').doc(d.id).delete().catch(() => {});
                        bizDocsDeleted++;
                    }
                }
            } catch (eBiz) {
                console.warn('[adminPurgeInactive] Business doc delete warn:', eBiz.message);
            }
        ...
        try {
            await db.collection('audit_logs').add({
                action: 'PURGE_INACTIVE_ACCOUNTS_BATCH',
                performedByUid: callerUid,
                performedByEmail: request.auth.token ? request.auth.token.email : '',
                totalPurged: results.length,
                purgedEmails: results.map((r) => r.email).filter(Boolean),
                timestamp: admin.firestore.FieldValue.serverTimestamp()
            });
        } catch (eAudit) {
            console.warn('[adminPurgeInactive] Audit log write warn:', eAudit.message);
        }
...
/**
 * Broadcast FCM Web Push Notifications to all active subscribers in blog_subscribers collection
 */
exports.broadcastBlogPushNotification = onCall(
    { cors: true },
    async (request) => {
        const callerUid = request.auth ? request.auth.uid : null;
        if (!callerUid) {
            throw new HttpsError("unauthenticated", "Authentication required to broadcast notifications.");
        }

        // Verify Super Admin
        const callerDoc = await db.collection("users").doc(callerUid).get().catch(() => null);
        const callerData = callerDoc && callerDoc.exists ? callerDoc.data() : {};
        const isSuper = callerData.superAdmin === true || String(callerData.role || "").toUpperCase() === "SUPER_ADMIN" || (request.auth.token && (request.auth.token.role === "SUPER_ADMIN" || request.auth.token.admin === true));
        ...
        if (!postId || !title) {
            throw new HttpsError("invalid-argument", "Both postId and title are required.");
        }

        // 1. Fetch active subscriber tokens from Firestore
        const subsSnap = await db.collection("blog_subscribers").where("active", "==", true).get().catch(() => ({ docs: [] }));
        const tokensMap = new Map();
        subsSnap.forEach((doc) => {
            const d = doc.data() || {};
            const token = d.fcmToken || doc.id;
            if (token && typeof token === "string" && token.length > 20) {
                tokensMap.set(token, doc.id);
            }
        });
        ...
        if (allTokens.length === 0) {
            return {
                success: true,
                message: "No active subscribers found in blog_subscribers collection.",
                sentCount: 0,
                totalSubscribers: 0
            };
        }

        logger.info(`[FCM Broadcast] Broadcasting post "${title}" (${postId}) to ${allTokens.length} subscribers.`);

        const targetUrl = `https://digibiz-sys.web.app/blog.html?id=${postId}`;
        const notificationBody = desc 
            ? (desc.length > 120 ? desc.slice(0, 117) + "..." : desc) 
            : "DIGIBIZ වෙතින් නව පද්ධති යාවත්කාලීනයක් හෝ භාවිත උපදෙසක් පළ විය. කියවීමට පිවිසෙන්න.";

        // Multicast in batches of up to 500 tokens
        ...
        for (let i = 0; i < allTokens.length; i += BATCH_SIZE) {
            ...
            const message = {
                ...
                webpush: {
                    ...
                    notification: {
                        icon: "https://digibiz-sys.web.app/icons/icon-192.svg",
                        badge: "https://digibiz-sys.web.app/icons/icon-192.svg",
                        vibrate: [200, 100, 200]
                    }
                }
            };
                    ...
                    response.responses.forEach((resp, idx) => {
                        if (!resp.success) {
                            ...
                            if (
                                ...
                                if (docId) invalidTokens.push(docId);
                            }
                        }
                    });
                }
            } catch (err) {
                logger.error("[FCM Broadcast] Batch send error:", err);
        ...
        if (invalidTokens.length > 0) {
            try {
                ...
                invalidTokens.forEach((docId) => {
                    batch.update(db.collection("blog_subscribers").doc(docId), { active: false, unsubscribedAt: admin.firestore.FieldValue.serverTimestamp() });
                });
                await batch.commit();
                logger.info(`[FCM Broadcast] Deactivated ${invalidTokens.length} stale tokens.`);
            } catch (eClean) {
                logger.warn("[FCM Broadcast] Token cleanup warning:", eClean);
            }
        }

        // Update blog post document with push notification metadata
        try {
            await db.collection("blog_posts").doc(postId).update({
                pushBroadcastedAt: admin.firestore.FieldValue.serverTimestamp(),
                pushBroadcastSuccessCount: successCount,
                pushBroadcastFailureCount: failureCount
            });
        } catch (eDoc) {
            logger.warn("[FCM Broadcast] Post doc update warning:", eDoc);
        }

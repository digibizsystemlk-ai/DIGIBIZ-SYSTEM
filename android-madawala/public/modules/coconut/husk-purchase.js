...
async function handleSaveHuskPurchase(e) {
    ...
    try {
        ...
        // 4. If CREDIT or CHEQUE purchase, update Supplier Balance & Sub-collection Ledger
        if ((paymentMode === 'CREDIT' || paymentMode === 'CHEQUE') && supplierId) {
            ...
            batch.set(supLedgerRef, {
                ...
                amount: totalCost,
                balanceAfter: newBal,
                description: `${paymentMode === 'CHEQUE' ? 'Cheque' : 'Credit'} Husk Purchase: ${huskCount.toLocaleString()} husks`,
                date: window.CoconutModule.tsToFirestore(pDateObj),
                createdAt: window.CoconutModule.tsToFirestore(new Date())
            });
        }
...
async function handleDeleteHuskPurchase(purchaseId) {
    ...
    try {
        ...
        // 2. Reverse Supplier Balance if Credit or Cheque
        if ((paymentMode === 'CREDIT' || paymentMode === 'CHEQUE') && supplierId) {
            const supRef = db.collection('coconut_suppliers').doc(supplierId);
...
async function handleSaveQuickSupplier(e) {
    ...
    try {
        ...
        window.CoconutModule.showToast('Supplier added successfully!', 'success');

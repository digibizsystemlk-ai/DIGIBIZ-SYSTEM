...
async function loadPaymentLogs() {
    ...
    try {
        ...
        if (!allPaymentLogs.length) {
            body.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:20px; color:var(--c-text-muted);">No payment settlements recorded yet.</td></tr>';
            return;
        }
            ...
            return `
                <tr>
                    ...
                    <td class="text-center">
                        <button class="c-btn c-btn-danger c-btn-sm" onclick="handleDeletePayment('${p.id}')" title="Delete & reverse balance/GL">🗑️</button>
                    </td>
                </tr>
            `;
        }).join('');
...
async function handleDeletePayment(payId) {
    const p = allPaymentLogs.find(x => x.id === payId);
    if (!p) return;

    const isReceipt = p.type === 'CUSTOMER_RECEIPT';
    if (!confirm(`⚠️ Are you sure you want to reverse this payment?\n\nType: ${p.type}\nParty: ${p.partyName}\nAmount: Rs. ${window.CoconutModule.fmt(p.amount, 2)}\n\nThis will restore outstanding balance and delete the journal entry.`)) {
        return;
    }
    ...
    try {
        ...
        if (isReceipt && p.partyId) {
            const cRef = db.collection('coconut_customers').doc(p.partyId);
        ...
        } else if (!isReceipt && p.partyId) {
            const sRef = db.collection('coconut_suppliers').doc(p.partyId);
        ...
        const payRef = db.collection('coconut_payments').doc(payId);
        batch.set(payRef, { isActive: false, deletedAt: window.CoconutModule.tsToFirestore(new Date()) }, { merge: true });

        await window.CoconutModule.deleteJournalForRef(bid, `coconut_payments/${payId}`);
        await window.CoconutModule.deleteJournalForRef(bid, payId);

        await batch.commit();

        window.CoconutModule.showToast('✅ Payment reversed successfully!', 'success');
        await loadParties();
        await loadPaymentLogs();

    } catch (e) {
        console.error(e);
        window.CoconutModule.showToast('Failed to delete payment: ' + e.message, 'error');
    }
}

...
(function () {
    ...
    function getSinhalaBusinessType(type) {
        ...
        if (t.includes('scrap')) return '♻️ පරණ යකඩ හා ලෝහ (Scrap)';
        if (t.includes('grocery') || t.includes('retail') || t.includes('pos')) return '🛒 සිල්ලර / තොග වෙළඳාම (Retail)';
        if (t.includes('bakery')) return '🍞 බේකරි නිෂ්පාදන (Bakery)';
        if (t.includes('pharmacy')) return '💊 ඖෂධ සැල් (Pharmacy)';
        if (t.includes('restaurant') || t.includes('cafe') || t.includes('food')) return '🍽️ ආපනශාලා / හෝටල් (Restaurant)';

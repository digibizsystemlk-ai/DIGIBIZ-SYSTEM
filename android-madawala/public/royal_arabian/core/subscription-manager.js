...
class SubscriptionManager {
    ...
    async loadOrCreateSubscription(user, businessId) {
        ...
        if (plan === 'PRO' || plan === 'ENTERPRISE' || plan === 'PAID') {
            try {
                const DEFAULT_VERSION_TAG = 'STD';
                const DEFAULT_SNAPSHOT_PATH = '/std/';
                const DEFAULT_FREEZE_DATE = 'STANDARD_RELEASE';

                const existingBizLock = businessData.lockedVersionTag || DEFAULT_VERSION_TAG;
                const existingPath = businessData.snapshotPath || (existingBizLock === 'STD' ? '/std/' : DEFAULT_SNAPSHOT_PATH);

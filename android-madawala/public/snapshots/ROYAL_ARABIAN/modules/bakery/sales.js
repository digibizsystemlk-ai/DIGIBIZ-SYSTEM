...
document.getElementById('mfgStyle').textContent = BakeryModule.baseStyles;
...
async function loadAreaOptions() {
    const bid = BakeryModule.businessId;
    if (!bid) return;
    try {
        const areaSuggestions = await BakeryModule.loadFieldSuggestions('mfg_area').catch(() => []);
...
async function loadFinishedProductsOptions() {
    ...
    if (!bid) return;
    try {
        const snap = await db.collection('bakery_finished_products')
...
async function updateSalesSummaryCards() {
    ...
    if (!bid) return;
    try {
        const snap = await db.collection('bakery_sales')
            .where('businessId', '==', bid)
            .get()
            .catch(() => ({ docs: [] }));
...
async function saveMfgSale() {
    ...
    try {
        ...
        if (customerArea) {
            await BakeryModule.saveFieldSuggestion('mfg_area', customerArea).catch(() => {});
        }
        ...
        await db.collection('bakery_sales').doc(saleId).set(salePayload);
        ...
        if (customerName) {
            ...
            await db.collection('bakery_route_plans').doc(routeDocId).set({
                businessId: bid,
        ...
        for (const item of saleCartItemsArray) {
            await db.collection('bakery_finished_products').doc(item.id).set({
        ...
        // 5. Mirror accounting
        try {
            await BakeryModule.syncFlatAccountingFinishedGoodSale(salePayload);
            await db.collection('bakery_sales').doc(saleId).update({ flatAccountingSyncedV1: true });
        } catch (eFlat) {
...
async function loadMfgSalesHistory() {
    ...
    try {
        ...
        tbody.innerHTML = rows.map(x => {
            ...
            return `
                <tr>
                    <td>${BakeryModule.formatDate(x.createdAt)}</td>
                    <td>
...
function printMfgSaleA5(saleId) {
    ...
    const bizName = (BakeryModule.context && BakeryModule.context.businessName) || 'DIGIBIZ MANUFACTURER';
    document.getElementById('a5BizName').textContent = bizName;
    document.getElementById('a5SaleId').textContent = sale.saleId || sale.id || '-';
    document.getElementById('a5Date').textContent = BakeryModule.formatDate(sale.createdAt);
...
function sendMfgSaleWhatsApp(saleId) {
    ...
    const bizName = (BakeryModule.context && BakeryModule.context.businessName) || 'DIGIBIZ MANUFACTURER';
    const dateStr = BakeryModule.formatDate(sale.createdAt);
...
async function deleteMfgSale(id) {
    ...
    try {
        const docRef = db.collection('bakery_sales').doc(id);
        const snap = await docRef.get();
        if (snap.exists) {
            const data = snap.data() || {};
            const bid = data.businessId || BakeryModule.businessId;

            // STOCK REVERSAL: Restore sold finished goods stock back to inventory!
            if (data.items && data.items.length > 0) {
                for (const item of data.items) {
                    if (item.productId && item.qty > 0) {
                        await db.collection('bakery_finished_products').doc(item.productId).set({
                            stockQty: firebase.firestore.FieldValue.increment(Math.abs(item.qty)),
                            updatedAt: new Date()
                ...
                await db.collection('bakery_finished_products').doc(`${bid}_${fgKey}`).set({

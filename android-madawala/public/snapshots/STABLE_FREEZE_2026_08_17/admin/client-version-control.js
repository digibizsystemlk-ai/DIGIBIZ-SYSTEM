...
(function () {
    ...
    async function handleSaveVersionConfig(e) {
        ...
        const snapshotPath = isDevMode ? '' : (dateMatch ? `/v${dateMatch[1]}/` : `/snapshots/${versionTag}/`);
        ...
        try {
            ...
            alert(isDevMode ? `🔓 Client ${email} released to LIVE Master Development (LATEST_DEV)!` : `🔒 Client Version Lock & Snapshot Path (/v${dateMatch ? dateMatch[1] : versionTag}/) successfully saved for ${email}!`);
            loadAllVersionConfigs();
        } catch (e) {

...
(function () {
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', function () {
            ...
            navigator.serviceWorker.register('/sw.js?v=3205').then(function(reg) {
    ...
    window.evaluateSandboxRouting = function() {
        try {
            ...
            const snapshotMatch = currentPath.match(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//);
            const isAlreadyInSnapshot = !!snapshotMatch;

            if (!config || currentPath.startsWith('/auth/') || currentPath.includes('/auth/login')) {
                // Do not sandbox login / authentication portal
                return;
            }

            const isLocked = !!(config.isLocked || config.lockStatus === 'LOCKED') && config.versionTag !== 'LATEST_DEV';
            const versionTag = config.versionTag || 'STABLE_FREEZE_2026_08_17';

            if (isLocked && versionTag !== 'LATEST_DEV') {
                const targetSnapshotPrefix = getSnapshotPrefix(versionTag, config.snapshotPath);
                if (!isAlreadyInSnapshot && (currentPath.startsWith('/modules/') || currentPath.startsWith('/admin/'))) {
                ...
                } else if (isAlreadyInSnapshot && targetSnapshotPrefix && !currentPath.startsWith(targetSnapshotPrefix)) {
                    // Client locked to a different snapshot version than current path
                    const purePath = currentPath.replace(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//, '');
                    const targetUrl = targetSnapshotPrefix + purePath + window.location.search + window.location.hash;
                    console.log('[SandboxGate] 🔒 Switching locked client to assigned snapshot: ' + targetUrl);
                    window.location.replace(targetUrl);
                }
            } else {
                if (isAlreadyInSnapshot) {
                    const livePath = currentPath.replace(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//, '/');

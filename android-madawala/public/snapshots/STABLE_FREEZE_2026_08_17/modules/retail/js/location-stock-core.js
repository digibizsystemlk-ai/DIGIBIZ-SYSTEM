    ...
    const LocationStockCore = {
        ...
        async receiveGRNTransactional({ businessId, po, receiveItems, currentUser }) {
            ...
            return await db.runTransaction(async (transaction) => {
                ...
                // 1a. Product and Location Stock Reads
                const productReads = [];
                const locStockReads = [];
                ...
                for (let i = 0; i < receiveItems.length; i++) {
                    ...
                    const currentProdData = (prodSnapObj && prodSnapObj.snap.exists) ? prodSnapObj.snap.data() : {};
                    const currentLocData = (locSnapObj && locSnapObj.snap.exists) ? locSnapObj.snap.data() : {};
        ...
        async reverseGRNTransactional({ businessId, poId, reversalReason, currentUser, userProfile }) {
            ...
            return await db.runTransaction(async (transaction) => {
                ...
                // 1c. Product & Location Stock Reads
                const productReads = [];
                const locStockReads = [];
                ...
                // Create Corrective Reversal Journal Entry

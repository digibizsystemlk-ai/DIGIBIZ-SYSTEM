...
(function() {
    ...
    // Do not show on landing page, blog, auth pages, attendance QR scan pages, or distributor/van rep apps
    const currentPath = window.location.pathname.toLowerCase();
    const isLanding = currentPath === '' || currentPath === '/' || currentPath.endsWith('/index.html') || currentPath.endsWith('index.html');
    const isBlog = currentPath.includes('blog.html') || currentPath.includes('/blog');
    const isAuth = currentPath.includes('/auth/');
    const isMobileScanner = currentPath.includes('mobile-scan.html') || currentPath.includes('repapp.html') || currentPath.includes('distributor-repapp');

    if (isLanding || isBlog || isAuth || isMobileScanner) {
        return;
    }

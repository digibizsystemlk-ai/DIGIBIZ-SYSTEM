...
window.AutoCareUtils = {
    ...
    upsertBusinessCollectionItem: async function (collectionName, docId, payload) {
        if (!collectionName || !docId || !payload) return null;
        const bizId = this.getBizId();
        ...
        try {
            ...
            await fs.collection('businesses').doc(bizId).collection(collectionName).doc(String(docId)).set(docPayload, { merge: true });
            return docId;
        } catch (e) {
            console.warn('[AutoCareUtils] Firestore upsert error for', collectionName, docId, e);
            return null;
        }
    },

    deleteBusinessCollectionItem: async function (collectionName, docId) {
        if (!collectionName || !docId) return false;
        const bizId = this.getBizId();
        const fs = this.getFirestore();
        if (!fs) return false;
        try {
            await fs.collection('businesses').doc(bizId).collection(collectionName).doc(String(docId)).delete();
            return true;
        } catch (e) {
            console.warn('[AutoCareUtils] Firestore delete error for', collectionName, docId, e);
            return false;
        }
    },

    saveJobFirestore: async function (jobObj) {
        if (!jobObj || !jobObj.id) return null;
        return this.upsertBusinessCollectionItem('autocare_jobs', jobObj.id, jobObj);
    },

    savePartFirestore: async function (partObj) {
        if (!partObj) return null;
        const docId = String(partObj.id || partObj.no || partObj.code || 'part').replace(/[\/\#\?]/g, '_');
        return this.upsertBusinessCollectionItem('autocare_parts', docId, partObj);
    },

    saveFinanceFirestore: async function (finObj) {
        if (!finObj || !finObj.id) return null;
        return this.upsertBusinessCollectionItem('autocare_finance', finObj.id, finObj);
    }
};

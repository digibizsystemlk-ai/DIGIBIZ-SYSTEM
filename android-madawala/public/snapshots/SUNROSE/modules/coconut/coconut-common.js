...
window.CoconutModule = (function () {
    ...
    Mod.deleteJournalForRef = async function (businessId, refString) {
        const db = Mod.getDb();
        if (!db || !businessId || !refString) return;
        try {
            ...
            console.warn('[CoconutModule] Error deleting journal for ref:', refString, e);
        }
    };
    ...
    Mod.getMetrics = async function (context) {
        ...
        // Read Point-Read Counter Aggregates (Cost Optimization with Automatic Fallback)
        let dailyAgg = null;
        let monthlyAgg = null;
        if (window.AggregateUtils) {
            try {
                [dailyAgg, monthlyAgg] = await Promise.all([
                    window.AggregateUtils.getDailyAggregate(db, bid),
                    window.AggregateUtils.getMonthlyAggregate(db, bid)
                ]);
            } catch (eAgg) {
                console.warn('[CoconutModule] Aggregates read error, fallback active:', eAgg);
            }
        }

        // 1. Coconut Purchases & Husk Purchases (Authoritative Live Calculation)
        ...
        coconutPurchasesSnap.docs.forEach(doc => {
            const data = doc.data() || {};
            if (data.isActive === false || data.status === 'DELETED') return;
        ...
        const huskData = huskStockDoc.exists ? huskStockDoc.data() : {};
        const huskStockQty = Number(huskData.stockHuskCount || huskData.stockQtyKg || huskData.stockKg) || 0;
        const huskAvgCost = Number(huskData.avgCostPerHusk || huskData.avgCostPerKg || huskData.lastCostPerHusk) || 0;
        rmStockValue += (huskStockQty * huskAvgCost);

        let fgStockValue = 0;
        finishedProductsSnap.docs.forEach(doc => {
            const data = doc.data() || {};
            const q = Number(data.stockQty) || 0;
            const c = Number(data.unitCost || data.lastCost) || 0;
            fgStockValue += (q * c);
        });

        // 3. Production Runs (Authoritative Live Calculation)
        ...
        // 4. Sales & COGS (Authoritative Live Calculation)
        let todaySales = 0;
        let monthSales = 0;
        let monthCogs = 0;
        ...
        // 5. Expenses (Authoritative Live Calculation)
        ...
        const supplierPayables = -Mod.calcAccountBalance(journalEntries, '2-2010-01') - Mod.calcAccountBalance(journalEntries, '2-2020-01');
    ...
    // --- Dynamic Reconciled Parties (Suppliers & Customers) with Auto-Healing ---
    Mod.loadReconciledSuppliers = async function (businessId) {
        const db = Mod.getDb();
        if (!db || !businessId) return [];

        const [supSnap, cpSnap, hpSnap, paySnap] = await Promise.all([
            db.collection('coconut_suppliers').where('businessId', '==', businessId).get(),
            db.collection('coconut_purchases').where('businessId', '==', businessId).get(),
            db.collection('coconut_husk_purchases').where('businessId', '==', businessId).get(),
            db.collection('coconut_payments').where('businessId', '==', businessId).get()
        ]);

        const suppliers = supSnap.docs
        ...
        suppliers.forEach(s => {
            s.calculatedBalance = 0;
            s.totalCreditPurchases = 0;
            s.totalPaymentsDisbursed = 0;
            supById.set(s.id, s);
            if (s.name) supByName.set(s.name.trim().toLowerCase(), s);
        });

        // 1. Coconut Purchases
        cpSnap.docs.forEach(d => {
            const p = d.data() || {};
            if (p.isActive !== false && (p.paymentMode === 'CREDIT' || p.paymentMode === 'CHEQUE')) {
                const amt = Number(p.totalCost) || 0;
                let targetSup = null;
                if (p.supplierId && supById.has(p.supplierId)) targetSup = supById.get(p.supplierId);
                else if (p.supplierName && supByName.has(p.supplierName.trim().toLowerCase())) targetSup = supByName.get(p.supplierName.trim().toLowerCase());

                if (targetSup) {
                    targetSup.calculatedBalance += amt;
                    targetSup.totalCreditPurchases += amt;
                }
            }
        });
        ...
        // 3. Payments Disbursed
        paySnap.docs.forEach(d => {
            const p = d.data() || {};
            if (p.isActive !== false && p.type === 'SUPPLIER_PAYMENT') {
                const amt = Number(p.amount) || 0;
                let targetSup = null;
                if (p.partyId && supById.has(p.partyId)) targetSup = supById.get(p.partyId);
                else if (p.partyName && supByName.has(p.partyName.trim().toLowerCase())) targetSup = supByName.get(p.partyName.trim().toLowerCase());

                if (targetSup) {
                    targetSup.calculatedBalance -= amt;
                    targetSup.totalPaymentsDisbursed += amt;
                }
            }
        });

        // Auto-heal Firestore document balance if out of sync
        const batch = db.batch();
        let needBatchCommit = false;

        suppliers.forEach(s => {
            const trueBal = Math.max(0, s.calculatedBalance);
            const currentDocBal = Number(s.balance) || 0;
            s.balance = trueBal; // Always set memory balance to true reconciled balance!

            if (Math.abs(trueBal - currentDocBal) > 0.01) {
                batch.set(db.collection('coconut_suppliers').doc(s.id), {
                    balance: trueBal,
                    updatedAt: Mod.tsToFirestore(new Date())
                }, { merge: true });
        ...
        if (needBatchCommit) {
            batch.commit().catch(e => console.warn('Supplier balance auto-heal sync error:', e));
        }

        return suppliers;
    };

    Mod.loadReconciledCustomers = async function (businessId) {
        const db = Mod.getDb();
        if (!db || !businessId) return [];

        const [custSnap, salesSnap, paySnap] = await Promise.all([
            db.collection('coconut_customers').where('businessId', '==', businessId).get(),
            db.collection('coconut_sales').where('businessId', '==', businessId).get(),
            db.collection('coconut_payments').where('businessId', '==', businessId).get()
        ]);
        ...
        customers.forEach(c => {
            c.calculatedBalance = 0;
            c.totalCreditSales = 0;
            c.totalPaymentsReceived = 0;
            custById.set(c.id, c);
            if (c.name) custByName.set(c.name.trim().toLowerCase(), c);
        });

        // 1. Sales
        salesSnap.docs.forEach(d => {
            const s = d.data() || {};
            if (s.isActive !== false && (s.paymentMode === 'CREDIT' || s.paymentMode === 'CHEQUE')) {
                const amt = Number(s.amount || s.totalAmount) || 0;
                let targetCust = null;
                if (s.customerId && custById.has(s.customerId)) targetCust = custById.get(s.customerId);
                else if (s.customerName && custByName.has(s.customerName.trim().toLowerCase())) targetCust = custByName.get(s.customerName.trim().toLowerCase());

                if (targetCust) {
                    targetCust.calculatedBalance += amt;
                    targetCust.totalCreditSales += amt;
                }
            }
        });
        ...
        paySnap.docs.forEach(d => {
            const p = d.data() || {};
            if (p.isActive !== false && p.type === 'CUSTOMER_RECEIPT') {
                const amt = Number(p.amount) || 0;
                let targetCust = null;
                if (p.partyId && custById.has(p.partyId)) targetCust = custById.get(p.partyId);
                else if (p.partyName && custByName.has(p.partyName.trim().toLowerCase())) targetCust = custByName.get(p.partyName.trim().toLowerCase());

                if (targetCust) {
                    targetCust.calculatedBalance -= amt;
                    targetCust.totalPaymentsReceived += amt;
                }
            }
        });
        ...
        customers.forEach(c => {
            const trueBal = Math.max(0, c.calculatedBalance);
            const currentDocBal = Number(c.balance) || 0;
            c.balance = trueBal;

            if (Math.abs(trueBal - currentDocBal) > 0.01) {
                batch.set(db.collection('coconut_customers').doc(c.id), {
                    balance: trueBal,
                    updatedAt: Mod.tsToFirestore(new Date())
                }, { merge: true });
                needBatchCommit = true;
        ...
        if (needBatchCommit) {
            batch.commit().catch(e => console.warn('Customer balance auto-heal sync error:', e));
        }

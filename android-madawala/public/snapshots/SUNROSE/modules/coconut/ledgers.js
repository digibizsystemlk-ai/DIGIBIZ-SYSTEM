...
async function loadInitialParties() {
    ...
    try {
        const [custList, supList] = await Promise.all([
            window.CoconutModule.loadReconciledCustomers(bid),
            window.CoconutModule.loadReconciledSuppliers(bid)
        ]);

        allCustomers = custList.filter(c => c.isActive !== false);
        allSuppliers = supList.filter(s => s.isActive !== false);
...
async function populatePartyDropdown() {
    ...
    if (type === 'customer') {
        ...
        select.innerHTML = '<option value="">Select Supplier...</option>' +
            allSuppliers.map(s => `<option value="${s.id}">${window.CoconutModule.esc(s.name)} ${s.area || s.estateLocation ? '(' + window.CoconutModule.esc(s.area || s.estateLocation) + ')' : ''} [Bal: Rs.${window.CoconutModule.fmt(s.balance)}]</option>`).join('');
    }
}
...
async function loadPartyStatement() {
    ...
    try {
        ...
        if (partyObj) {
            ...
            document.getElementById('statementPartyMeta').textContent = `Phone: ${partyObj.phone || '-'} | Location: ${partyObj.area || partyObj.estateLocation || '-'}`;
        ...
        // If no explicit subcollection ledger docs found, dynamically build from active transactions
        if (!entries.length && partyObj) {
            const pName = (partyObj.name || '').trim().toLowerCase();
            if (type === 'supplier') {
                const [cpSnap, hpSnap, paySnap] = await Promise.all([
                ...
                cpSnap.docs.forEach(d => {
                    const p = d.data() || {};
                    if (p.isActive !== false && (p.supplierId === partyId || (p.supplierName && p.supplierName.trim().toLowerCase() === pName))) {
                        entries.push({
                            type: 'PURCHASE',
                            referenceId: d.id,
                            amount: Number(p.totalCost) || 0,
                            description: `Coconut Purchase (${p.quantity || 0} nuts) [${p.paymentMode || 'CASH'}]`,
                ...
                hpSnap.docs.forEach(d => {
                    ...
                    if (p.isActive !== false && (p.supplierId === partyId || (p.supplierName && p.supplierName.trim().toLowerCase() === pName))) {
                        entries.push({
                            ...
                            referenceId: d.id,
                            amount: Number(p.totalCost) || 0,
                            description: `Husk Purchase (${p.huskCount || p.quantityKg || 0} husks) [${p.paymentMode || 'CASH'}]`,
                            date: p.date || p.createdAt
                        });
                    }
                });
                ...
                paySnap.docs.forEach(d => {
                    const p = d.data() || {};
                    if (p.isActive !== false && p.type === 'SUPPLIER_PAYMENT' && (p.partyId === partyId || (p.partyName && p.partyName.trim().toLowerCase() === pName))) {
                        entries.push({
                            type: 'PAYMENT_DISBURSED',
                            referenceId: d.id,
                            amount: Number(p.amount) || 0,
                            description: `Payment Disbursed (${p.paymentMode || 'CASH'}): ${p.notes || 'Settlement'}`,
                            date: p.date || p.createdAt
                        });
                    }
                });
                ...
                const [salesSnap, paySnap] = await Promise.all([
                ...
                salesSnap.docs.forEach(d => {
                    const s = d.data() || {};
                    if (s.isActive !== false && (s.customerId === partyId || (s.customerName && s.customerName.trim().toLowerCase() === pName))) {
                        entries.push({
                            type: 'SALE',
                            referenceId: d.id,
                            amount: Number(s.amount || s.totalAmount) || 0,
                            description: `Sale Invoice [${s.paymentMode || 'CASH'}]`,
                ...
                paySnap.docs.forEach(d => {
                    const p = d.data() || {};
                    if (p.isActive !== false && p.type === 'CUSTOMER_RECEIPT' && (p.partyId === partyId || (p.partyName && p.partyName.trim().toLowerCase() === pName))) {
                        entries.push({
                            type: 'PAYMENT_RECEIVED',
                            referenceId: d.id,
                            amount: Number(p.amount) || 0,
                            description: `Payment Received (${p.paymentMode || 'CASH'}): ${p.notes || 'Settlement'}`,
                            date: p.date || p.createdAt
                        });
                    }
                });

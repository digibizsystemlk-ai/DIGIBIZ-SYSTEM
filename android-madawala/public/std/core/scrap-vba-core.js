...
(function () {
    ...
    async function enqueuePendingSms(businessId, phone, message) {
        ...
        const activeApiKey = smsSettingsData.apiKey || (window.SMS_API_CONFIG && window.SMS_API_CONFIG.apiKey);

        // 1. If valid REST API key exists, try sending via REST API
        if (activeApiKey && activeApiKey !== 'YOUR_API_KEY_HERE') {
            try {
                ...
                if (window.SMS_API_CONFIG) {
                    window.SMS_API_CONFIG.apiKey = activeApiKey;
                }
                if (typeof window.sendSMSViaAPI === 'function') {
                    ...
                    if (res && res.success) {
                        ...
                        smsDebug(`SMS sent via REST API → ${mobile}`, true);
                        return { ok: true, via: "rest_api", id: res.messageId, postBalance: res.creditsRemaining };
                    }
                }
            } catch (errApi) {
                console.warn("[SMS] REST API attempt warning:", errApi);
            }
        }

        // 2. Guaranteed Gateway: Queue to Firestore pending_sms AND Realtime Database sms_gateway for Android Phone / SIM Gateway
        try {
            const smsDocId = `sms_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
            const payload = {
                businessId: bizIdStr,
                mobile: mobile,
                message: finalBrandedText,
            ...
            if (window.db) {
                await window.db.collection('pending_sms').doc(smsDocId).set(payload).catch(e => console.warn('Firestore pending_sms set:', e));
            }

            if (typeof firebase !== 'undefined' && typeof firebase.database === 'function') {
                await firebase.database().ref(`sms_gateway/${bizIdStr}/pending_sms`).child(smsDocId).set({
                    businessId: bizIdStr,
                    mobile: mobile,
                    message: finalBrandedText,
                    createdAt: Date.now()
                }).catch(e => console.warn('RTDB sms_gateway set:', e));
            }

            smsDebug(`SMS queued successfully (Gateway/SIM) → ${mobile}`, true);
            return { ok: true, via: "android_gateway", id: smsDocId };
        } catch (errQueue) {
            console.error("[SMS] Gateway queue error:", errQueue);
            smsDebug(`SMS queue failed → ${mobile}: ${errQueue.message || errQueue}`, false);
            return { ok: false, error: String(errQueue) };
        }
    }
    ...
    window.scrapVbaCore = {
        ...
        recalculateProfitPoolFromHistory: async function(businessId) {
            ...
            const [itemsSnap, buySnap, revSnap, expSnap] = await Promise.all([
                window.db.collection('scrap_items').where('businessId', '==', businessId).get().catch(() => ({ docs: [] })),

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function calculate7DayActivityScore(bid, ownerUid, lastLoginDate, userData = {}) {
        ...
        try {
            ...
            const collectionsToScan = [
                ...
                'bakery_sales', 'bakery_production_batches', 'bakery_raw_materials', 'bakery_finished_products', 'bakery_expenses',
    ...
    extendProBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            await window.db.collection('businesses').doc(businessId).set({
                status: 'ACTIVE',
    ...
    if (deleteAccountBtn) {
        deleteAccountBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // 3. Purge subcollections under target businessId / ownerUserId
                const subcollectionDefs = [
                    { root: 'journal', sub: 'entries' },
                    { root: 'purchases', sub: 'orders' },
                    { root: 'products', sub: 'list' },
                    { root: 'customers', sub: 'list' },
                    { root: 'suppliers', sub: 'list' },
                    { root: 'banks', sub: 'accounts' },
                    { root: 'banks', sub: 'transactions' },
                    { root: 'expenses', sub: 'list' },
                    { root: 'orders', sub: 'list' }
                ];

                for (const idVal of targetIds) {
                    for (const def of subcollectionDefs) {
                        try {
                            const subSnap = await window.db.collection(def.root).doc(idVal).collection(def.sub).get().catch(() => ({ docs: [] }));
                            if (subSnap && subSnap.docs) {
                                subSnap.docs.forEach(d => deletePromises.push(d.ref.delete().catch(() => {})));
                            }
                        } catch (eSub) {}
                    }
                }

                // 4. Purge client_version_control document
                if (currentEmail) {
                    const vcDocId = String(currentEmail).trim().toLowerCase().replace(/[^a-z0-9@]/g, '_');
                    deletePromises.push(window.db.collection('client_version_control').doc(vcDocId).delete().catch(() => {}));
                }

                // 5. Purge all matching top-level collection records
                for (const idVal of targetIds) {
                ...
                // 6. Delete User from Firebase Authentication via Cloud Function
                try {
                    if (window.firebase && typeof window.firebase.functions === 'function') {
                        const deleteAuthFn = window.firebase.functions().httpsCallable('deleteUserAccountByEmail');
                        await deleteAuthFn({ targetEmail: currentEmail });
                    }
                } catch (eAuthFn) {
                    console.warn('[AccountWipe] Auth delete cloud function warning:', eAuthFn);
                }
    ...
    async function impersonateClientAccount(targetEmail, targetBizId, targetBizType) {
        ...
        const normType = String(resolvedBizType || 'retail').toLowerCase().trim();
        let destUrl = '/modules/retail/dashboard.html';
        if (normType === 'distributor') destUrl = '/modules/distributor/web/dashboard.html';
        else if (normType === 'attendance_payroll' || normType === 'attendance') destUrl = '/modules/attendance_payroll/dashboard.html';
        else if (normType === 'auto_care') destUrl = '/modules/auto_care/dashboard.html';
        else if (normType === 'bakery') destUrl = '/modules/bakery/dashboard.html';
        else if (normType === 'coconut') destUrl = '/modules/coconut/dashboard.html';
        else if (normType === 'hardware') destUrl = '/modules/hardware/dashboard.html';
        else if (normType === 'manufacturer') destUrl = '/modules/manufacturer/dashboard.html';
        else if (normType === 'pharmacy') destUrl = '/modules/pharmacy/dashboard.html';
        else if (normType === 'quick_billing') destUrl = '/modules/quick_billing/app.html';
        else if (normType === 'scrap_collection_center') destUrl = '/modules/scrap_collection_center/dashboard.html';
        else if (normType === 'tire_centre') destUrl = '/modules/tire_centre/dashboard.html';
        else destUrl = `/modules/${normType}/dashboard.html`;

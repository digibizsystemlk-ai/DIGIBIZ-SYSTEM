...
class DashboardCore {
    ...
    getVerticalDashboardUrl(typeRaw) {
        const rawCheck = String(typeRaw || '').toLowerCase();
        if (rawCheck.includes('micro_tech') || rawCheck.includes('microtech')) return '/clients/micro_tech/index.html';
        const norm = this.normalizeBusinessType(typeRaw);
        if (norm === 'communication' || norm === 'microcom' || norm === 'microcommunication') return '/microcom#dashboard';
        if (norm === 'quick_billing') return '/modules/quick_billing/app.html';
        if (norm === 'distributor') return '/modules/distributor/web/dashboard.html';
        if (norm === 'bakery') return '/modules/bakery/dashboard.html';
    ...
    async getContext(user) {
        ...
        if (path.includes('/tire_centre/')) businessType = 'tire_centre';
        else if (path.includes('/distributor/')) businessType = 'distributor';
        else if (path.includes('/scrap_collection_center/')) businessType = 'scrap_collection_center';
        else if (path.includes('/pharmacy/')) businessType = 'pharmacy';
        else if (path.includes('/hardware/')) businessType = 'hardware';
        else if (path.includes('/service/')) businessType = 'service';
        else if (path.includes('/manufacturer/')) businessType = 'manufacturer';
        else if (path.includes('/bakery/')) businessType = 'bakery';
        else if (path.includes('/restaurant/')) businessType = 'restaurant';
        else if (path.includes('/garment/')) businessType = 'garment';
        else if (path.includes('/auto_care/')) businessType = 'auto_care';
        else if (path.includes('/coconut/')) businessType = 'coconut';
        else if (path.includes('/attendance_payroll/')) businessType = 'attendance_payroll';
        else if (userEmail.includes('tyrecentre') || userEmail.includes('tire')) businessType = 'tire_centre';
        else if (userEmail.includes('distributor')) businessType = 'distributor';
        else if (userEmail.includes('pharmacy')) businessType = 'pharmacy';
        else if (userEmail.includes('hardware')) businessType = 'hardware';
        else if (userEmail.includes('service')) businessType = 'service';
        else if (userEmail.includes('manufacturer')) businessType = 'manufacturer';
        else if (userEmail.includes('bakery')) businessType = 'bakery';
        else if (userEmail.includes('scrap')) businessType = 'scrap_collection_center';
        else if (userEmail.includes('coconut')) businessType = 'coconut';
        else if (userDocData && (userDocData.businessType || userDocData.type)) businessType = this.normalizeBusinessType(userDocData.businessType || userDocData.type);
        else if (storedBusinessType) businessType = storedBusinessType;
        ...
        if (isImpersonating) {
            ...
            businessType = this.normalizeBusinessType(localStorage.getItem('digibiz_impersonate_type') || localStorage.getItem('currentBusinessType') || businessType);
        ...
        if (businessDoc.exists) {
            ...
            if (bd.businessType) {
                businessType = this.normalizeBusinessType(bd.businessType);
            }
            businessName = bd.name || businessName;
            logoUrl = String(bd.logoUrl || '').trim();
        }
        if (path.includes('/tire_centre/')) {
            businessType = 'tire_centre';
        } else if (path.includes('/distributor/')) {
            businessType = 'distributor';
        } else if (path.includes('/scrap_collection_center/')) {
            businessType = 'scrap_collection_center';
        }
    ...
    async getRecentJournalActivities(businessId, limit = 5, businessType = '') {
        ...
        let snapshot;
        try {
            snapshot = await window.db.collection('journal').doc(businessId).collection('entries').get();
        } catch (e) {
            snapshot = { docs: [] };
        }
    ...
    async syncUnjournaledTransactions(bid, userEmailOrType, maybeEmail) {
        ...
        try {
            ...
            try {
                const supQueries = [
                    window.db.collection('suppliers').where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
                ...
                for (const doc of allSupDocs) {
                    ...
                    // Auto-create / verify mock PO in purchases/{bid}/orders
                    const poRef = window.db.collection('purchases').doc(bid).collection('orders').doc(`PO-OPEN-${sCode}`);
                    const poDoc = await poRef.get().catch(() => null);
    ...
    async getTireCentreMetrics(context) {
        ...
        try {
            const [custSnap1, custSnap2] = await Promise.all([
                ...
                window.db.collection('customers').where('businessId', '==', context.businessId).get().catch(() => ({ docs: [] }))
            ]);
            const custMap = {};
        ...
        try {
            const [supSnap1, supSnap2, supSnap3] = await Promise.all([
                ...
                window.db.collection('suppliers').where('businessId', '==', context.businessId).get().catch(() => ({ docs: [] })),
            ...
            if (!journalSnap.empty) {
                ...
                for (const code in glBalances) {
                    ...
                    if (code.startsWith('1-1030') || code === 'AC-10300') {
                        ...
                        if (!accName.includes('inventory') && !accName.includes('stock') && !accName.includes('තොග') && !accName.includes('asset')) {
        ...
        const finalCashBalance = hasGL ? Number(glCashBalance.toFixed(2)) : cashBalance;
        const finalBankBalance = hasGL ? Number(glBankBalance.toFixed(2)) : bankBalance;
    ...
    async getManufacturerMetrics(context) {
        ...
        const [rmSnap, fgSnap, prodSnap, prodRunsSnap, opSnap, sideSnap, journalSnap, mapSnap, payableSnap, receivableSnap, rawHistSnap, salesHistSnap] = await Promise.all([
            window.db.collection(`${collPrefix}raw_materials`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}finished_products`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}production_batches`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}production_runs`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}expenses`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}side_income`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection('journal').doc(bid).collection('entries').get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}transformations`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}raw_material_history`).where('businessId', '==', bid).where('paymentStatus', 'in', ['PENDING', 'PENDING_CLEARANCE']).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}sales`).where('businessId', '==', bid).where('paymentStatus', 'in', ['PENDING', 'PENDING_CLEARANCE']).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}raw_material_history`).where('businessId', '==', bid).get().catch(() => ({ docs: [] })),
            window.db.collection(`${collPrefix}sales`).where('businessId', '==', bid).get().catch(() => ({ docs: [] }))
        ]);

...
function setupEventHandlers() {
    ...
    const copraInput = document.getElementById('outCopraQty');
    ...
    function updateCalculations() {
        ...
        const c = Number(copraInput ? copraInput.value : 0) || 0;
        const s = Number(spoilInput.value) || 0;

        const totalSorted = g + m + l + c + s;
    ...
    if (copraInput) copraInput.addEventListener('input', updateCalculations);
...
async function loadGradingLogs() {
    ...
    try {
        ...
        body.innerHTML = runs.map(r => {
            ...
            const copraTxt = r.copraQty > 0 ? `<div style="color:#92400e; font-size:11px; font-weight:700;">+${window.CoconutModule.fmt(r.copraQty, 0)} Copra</div>` : '';
            return `
                <tr>
                    <td>${dt}</td>
                    ...
                    <td class="text-right" style="color:#b45309; font-weight:700;">+${window.CoconutModule.fmt(r.lowQty, 0)}${copraTxt}</td>
...
async function handleSaveGrading(e) {
    ...
    try {
        ...
        const cQty = Number(document.getElementById('outCopraQty') ? document.getElementById('outCopraQty').value : 0) || 0;
        const sQty = Number(document.getElementById('outSpoiledQty').value) || 0;
        const notes = document.getElementById('gNotes').value || '';

        const totalSorted = gQty + mQty + lQty + cQty + sQty;
        if (totalSorted <= 0) {
        ...
        for (const t of targets) {
            if (t.qty > 0) {
                ...
                batch.set(tRef, {
                    businessId: bid,
                    category: t.cat,
                    categoryName: t.cat === 'COPRA_NUTS' ? 'Copra Nuts (කොප්පරා සඳහා ගෙඩි)' : t.cat,
                    stockQty: newTQty,

...
function setupEventHandlers() {
    ...
    if (editModal) {
        document.getElementById('btnCloseEditExpenseModal').onclick = () => editModal.classList.remove('open');
        document.getElementById('btnCancelEditExpense').onclick = () => editModal.classList.remove('open');
        document.getElementById('editExpenseForm').addEventListener('submit', handleSaveEditExpense);
    }
            ...
            <tr>
                ...
                <td class="text-center">
                    <div style="display:inline-flex; gap:4px;">
                        <button class="c-btn c-btn-secondary c-btn-sm" onclick="openEditExpenseModal('${exp.id}')" title="Edit expense">✏️</button>
...
function openEditExpenseModal(expId) {
    const exp = allExpenses.find(x => x.id === expId);
    if (!exp) return;

    document.getElementById('editExpId').value = expId;
    document.getElementById('editExpDate').value = window.CoconutModule.toLocalDateStr(exp.date || exp.createdAt);
    document.getElementById('editExpCategory').value = exp.category || 'Other';
    document.getElementById('editExpAmount').value = exp.amount || 0;
    document.getElementById('editExpPaymentMode').value = exp.paymentMode || 'CASH';
...
async function handleSaveEditExpense(e) {
    e.preventDefault();
    const btn = document.getElementById('btnSaveEditExpense');
    btn.disabled = true;
    ...
    try {
        const newDateStr = document.getElementById('editExpDate').value;
        const newDateObj = window.CoconutModule.parseDateAny(newDateStr) || new Date();
        const newCategory = document.getElementById('editExpCategory').value;
        const newAmount = Number(document.getElementById('editExpAmount').value) || 0;
        const newMode = document.getElementById('editExpPaymentMode').value;
        const newDesc = document.getElementById('editExpDescription').value.trim();

        if (newAmount <= 0 || !newDesc) {
            alert('Please specify valid amount and description');
            btn.disabled = false;
            btn.textContent = '💾 Update & Sync GL';
            return;
        }
        ...
        // 2. Remove old Journal & Post New Balanced Journal
        await window.CoconutModule.deleteJournalForRef(bid, `coconut_expenses/${expId}`);
        await window.CoconutModule.deleteJournalForRef(bid, exp.expId || expId);

        const journalLines = [
            { accountCode: '5-5010-01', accountName: `Operational Expense (${newCategory})`, debit: newAmount, credit: 0 }
        ];
        if (newMode === 'BANK' || newMode === 'CHEQUE') {
            journalLines.push({ accountCode: '1-1020-01', accountName: 'Bank Account', debit: 0, credit: newAmount });
        } else {
            journalLines.push({ accountCode: '1-1010-01', accountName: 'Cash in Drawer', debit: 0, credit: newAmount });
        }

        await window.CoconutModule.postJournalEntry({
            businessId: bid,
            description: `Expense (Updated): ${newDesc} [${newCategory}]`,
        ...
        window.CoconutModule.showToast('✅ Expense updated & General Ledger synced!', 'success');
        document.getElementById('editExpenseModal').classList.remove('open');
        await loadExpenses();

    } catch (err) {
        console.error(err);
        window.CoconutModule.showToast('Failed to update expense: ' + err.message, 'error');
    } finally {
        btn.disabled = false;
        btn.textContent = '💾 Update & Sync GL';
    }

...
function setupEventHandlers() {
    ...
    document.getElementById('btnOpenNewProduct').onclick = () => {
        ...
        document.getElementById('prodUnitName').value = 'Liters';
...
async function loadProducts() {
    ...
    try {
        ...
        if (!allProducts.some(p => (p.name || '').toLowerCase().includes('copra') || (p.name || '').includes('කොප්පරා'))) {
            ...
            await db.collection('coconut_finished_products').doc(appCtx.businessId).collection('items').doc(copraId).set(copraObj, { merge: true });
    ...
    body.innerHTML = list.map(p => {
        ...
        return `
            <tr>
                ...
                <td>${window.CoconutModule.esc(p.unitName || 'Liters')}</td>
                <td class="text-right" style="font-weight:800; color:${isLow ? 'var(--c-danger)' : 'var(--c-text)'};">${window.CoconutModule.fmt(qty, 0)} ${p.unitName || 'L'}</td>
                ...
                <td class="text-center">
                    <span class="c-badge ${isLow ? 'c-badge-danger' : 'c-badge-success'}">${isLow ? 'Low Stock' : 'In Stock'}</span>
                </td>
                <td class="text-center">
...
function handleEditProduct(id) {
    ...
    document.getElementById('prodUnitName').value = p.unitName || 'Liters';

...
function populateItemDropdown() {
    ...
    const copraDivertRow = document.getElementById('coconutCopraDivertRow');
    if (copraDivertRow) {
        copraDivertRow.style.display = (type === 'COCONUT') ? 'block' : 'none';
    }

    if (type === 'COCONUT') {
        const categories = ['GOOD', 'MEDIUM', 'LOW', 'COPRA_NUTS', 'UNGRADED'];
        select.innerHTML = categories.map(cat => {
            const data = allCoconutStock[cat] || { qty: 0, cost: 0 };
            const label = cat === 'COPRA_NUTS' ? 'Copra Nuts (කොප්පරා ගෙඩි)' : `${cat} Grade`;
            return `<option value="${cat}">${label} (${data.qty} in stock)</option>`;
        }).join('');
    } else {
        if (!allProducts.length) {
            ...
            select.innerHTML = allProducts.map(p => {
                ...
                return `<option value="${p.id}">${window.CoconutModule.esc(p.name)} (${qty} ${p.unitName || 'Units'} in stock)</option>`;
            }).join('');
        }
    }
...
function updateItemPriceAndStock() {
    ...
    if (type === 'COCONUT') {
        ...
        const defaultRates = { GOOD: 120, MEDIUM: 100, LOW: 80, COPRA_NUTS: 85, UNGRADED: 95 };
        if (!priceInput.value || priceInput.dataset.autoFilled === 'true') {
            priceInput.value = defaultRates[itemKey] || Math.round((data.cost || 70) * 1.25);
            priceInput.dataset.autoFilled = 'true';
        }
    } else {
        const prod = allProducts.find(x => x.id === itemKey);
        if (prod) {
            availSpan.textContent = `${window.CoconutModule.fmt(prod.stockQty, 0)} ${prod.unitName || 'Units'}`;
            if (!priceInput.value || priceInput.dataset.autoFilled === 'true') {
...
function handleAddToCart() {
    ...
    const copraDivertQty = (type === 'COCONUT' && document.getElementById('cartCopraDivertQty'))
        ? (Number(document.getElementById('cartCopraDivertQty').value) || 0)
        : 0;
    ...
    const totalStockDeduction = qty + copraDivertQty;
    if (totalStockDeduction > availableQty) {
        if (!confirm(`⚠️ Total nuts to deduct (${totalStockDeduction} = ${qty} sold + ${copraDivertQty} to copra) exceeds available stock (${availableQty}). Add anyway?`)) return;
    }
    ...
    if (document.getElementById('cartCopraDivertQty')) document.getElementById('cartCopraDivertQty').value = '0';
    renderCart();
}
...
function renderCart() {
    ...
    body.innerHTML = cart.map((item, idx) => {
        ...
        const copraNote = (item.copraDivertQty > 0)
            ? `<div style="font-size:11px; color:#b45309; font-weight:700; margin-top:2px;">🥥 + ${window.CoconutModule.fmt(item.copraDivertQty, 0)} nuts diverted to Copra</div>`
            : '';

        return `
            <tr>
                <td><strong>${window.CoconutModule.esc(item.name)}</strong>${copraNote}</td>
...
async function handleSaveSale(e) {
    ...
    try {
        ...
        let totalCopraDivertedNuts = 0;
        let totalCopraDivertedCostVal = 0;

        // 1. Deduct Respective Inventory Items & Re-allocate Diverted Copra Nuts
        for (const item of cart) {
            if (item.itemType === 'COCONUT') {
                ...
                const totalDeduct = item.qty + (Number(item.copraDivertQty) || 0);
                const newQty = Math.max(0, cur.qty - totalDeduct);
                batch.set(catRef, {
                    stockQty: newQty,
                    updatedAt: window.CoconutModule.tsToFirestore(new Date())
                }, { merge: true });

                // If Copra nuts were diverted during this sale line
                const copraQty = Number(item.copraDivertQty) || 0;
                if (copraQty > 0) {
                    totalCopraDivertedNuts += copraQty;
                    const divertVal = copraQty * (item.cogsUnit || 0);
                    totalCopraDivertedCostVal += divertVal;
                    ...
                    const newCopraQty = curCopraQty + copraQty;
                    const newCopraVal = (curCopraQty * curCopraAvgCost) + divertVal;
                    const newCopraAvgCost = newCopraQty > 0 ? (newCopraVal / newCopraQty) : (item.cogsUnit || 0);
        ...
        batch.set(saleRef, {
            ...
            copraDivertedNuts: totalCopraDivertedNuts,
            copraDivertedCostVal: Number(totalCopraDivertedCostVal.toFixed(2)),
        ...
        // 5. If Copra nuts were diverted during sale, record inventory category transfer
        if (totalCopraDivertedCostVal > 0) {
            await window.CoconutModule.postJournalEntry({
                businessId: bid,
                description: `Sales Inspection Rejection Divert: ${totalCopraDivertedNuts} nuts transferred to Copra Raw Stock`,
                referenceType: 'COPRA_REJECTION_TRANSFER',
                ref: `coconut_sales/${saleId}`,
                date: sDateObj,
                lines: [
                    { accountCode: '1-1040-01', accountName: 'Inventory (Copra Raw Nuts)', debit: totalCopraDivertedCostVal, credit: 0 },
                    { accountCode: '1-1040-01', accountName: 'Inventory (Fresh Graded Coconuts)', debit: 0, credit: totalCopraDivertedCostVal }
                ],
                batch
            });
        }
        ...
        if (window.AggregateUtils) {
            window.AggregateUtils.bumpAggregates({
                businessId: bid,
                type: 'SALE',
                value: grandTotal,
                profit: profit,
                cost: grandCogs,
                count: 1
            }).catch(e => console.warn('Aggregates bump error:', e));
        }

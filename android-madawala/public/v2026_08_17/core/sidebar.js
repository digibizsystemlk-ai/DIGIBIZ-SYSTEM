...
class Sidebar {
    ...
    resolveSidebarLink(rawLink) {
        ...
        const match = pathname.match(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//);
        if (match) {
    ...
    isMenuActive(link, pathname) {
        ...
        const normalizedPath = (pathname || '').replace(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//, '/');
        const normalizedLink = (linkPathRaw || '').replace(/^\/(snapshots\/[^\/]+|v[0-9_]+)\//, '/');
        const cleanLink = normalizedLink.replace(/\/+/g, '');
        const cleanPath = normalizedPath.split('#')[0].split('?')[0].replace(/\/+/g, '');

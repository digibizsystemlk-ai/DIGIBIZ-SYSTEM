...
class SubscriptionManager {
    ...
    async loadOrCreateSubscription(user, businessId) {
        ...
        const isDemoUser = officialDemoEmails.includes(userEmail)
            || (userEmail.startsWith('test@') && userEmail.endsWith('.com') && userEmail !== 'test@bill.com')
            || (settingsData && settingsData.isDemo === true)
            || (subscription && subscription.plan === 'LIVE_DEMO');

        const expireDate = new Date(subscription.expireDate || subscription.trialEnd || now.toISOString());
        let remainingDays = Math.max(0, this.daysBetween(now, expireDate));
        ...
        if (plan === 'PRO' || plan === 'ENTERPRISE' || plan === 'PAID') {
            try {
                const DEFAULT_VERSION_TAG = 'STABLE_FREEZE_2026_08_17';
                const DEFAULT_SNAPSHOT_PATH = '/v2026_08_17/';
                const DEFAULT_FREEZE_DATE = '2026-08-17';

                const existingBizLock = businessData.lockedVersionTag || DEFAULT_VERSION_TAG;
                const existingPath = businessData.snapshotPath || (existingBizLock === 'STABLE_FREEZE_2026_08_11' ? '/v2026_08_11/' : DEFAULT_SNAPSHOT_PATH);

                window.db.collection('businesses').doc(businessId).set({
                    versionLock: true,
                    lockedVersionTag: existingBizLock,
                    snapshotPath: existingPath,
                    freezeDate: businessData.freezeDate || DEFAULT_FREEZE_DATE,
                    profileLocked: true
                }, { merge: true });
    ...
    async initializeForUser(user, role, businessId) {
        ...
        const isDemoUser = officialDemoEmails.includes(userEmail)
            || (userEmail.startsWith('test@') && userEmail.endsWith('.com') && userEmail !== 'test@bill.com');
        const isSuperAdmin = role === 'SUPER_ADMIN' || userEmail === 'superadmin@digibiz.lk';
        ...
        this.current = { ...subscription, isSuperAdmin, isDemoUser, businessId };

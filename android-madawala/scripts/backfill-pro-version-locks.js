...
async function backfillProVersionLocks() {
    ...
    const versionTag = process.argv[2] || 'STD';
    const isStd = versionTag === 'STD' || versionTag === 'STANDARD';
    const dateMatch = versionTag.match(/(\d{4}_\d{2}_\d{2})/);
    const snapshotPath = isStd ? '/std/' : (dateMatch ? `/v${dateMatch[1]}/` : `/snapshots/${versionTag}/`);
    const freezeDate = isStd ? 'STANDARD_RELEASE' : (dateMatch ? dateMatch[1].replace(/_/g, '-') : '2026-08-17');
    const flags = {

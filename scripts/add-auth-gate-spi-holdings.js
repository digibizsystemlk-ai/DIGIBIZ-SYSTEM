...
const filePath = path.resolve(__dirname, '../public/clients/spi_holdings/index.html');
...
const authGateHtml = `
    ...
    <div id="authGateOverlay">
        <div class="brand">
            ...
            SPI<span>HOLDINGS</span>
        </div>
        <div class="login-box">
            <h2>SPI HOLDINGS</h2>
            <p>Distributor Management Platform — Secure Sign In</p>
            <div class="error" id="authError"></div>
            <input type="email" id="loginEmail" placeholder="Owner / Authorized Email" value="nadundealwis@gmail.com" autocomplete="username" />
...
const authGateJs = `
        ...
        auth.onAuthStateChanged(function(user) {
            if (user) {
                ...
                const isAllowed = allowedEmails.some(e => e.toLowerCase() === userEmail) || 
                                  userEmail.includes('nadun') || 
                                  userEmail.includes('spiholdings') || 
                                  userEmail.includes('spi_holdings') || 
                                  userEmail.includes('dealwis') || 
                ...
                if (!isAllowed) {
                    ...
                    if (err) {
                        err.innerHTML = '<b>ACCESS DENIED:</b> Account <code>' + userEmail + '</code> is not authorized for SPI HOLDINGS.';
                ...
                hideAuthGate();
                localStorage.setItem('spi_holdings_auth_active', 'true');
                sessionStorage.setItem('spi_holdings_auth_active', 'true');
            } else {
                if (sessionStorage.getItem('spi_holdings_auth_active') !== 'true' && localStorage.getItem('spi_holdings_auth_active') !== 'true') {
                    showAuthGate();
                }
            }
        });
        ...
        async function handleSignIn() {
            ...
            try {
                ...
                localStorage.setItem('spi_holdings_auth_active', 'true');
                sessionStorage.setItem('spi_holdings_auth_active', 'true');
                showToast('Welcome to SPI HOLDINGS!');
            } catch (err) {
                if (loader) loader.style.display = 'none';
                if (btn) btn.disabled = false;
                if (errEl) {
        ...
        async function handleSignOut() {
            ...
            try {
                await auth.signOut();
                localStorage.removeItem('spi_holdings_auth_active');
                sessionStorage.removeItem('spi_holdings_auth_active');
                showAuthGate();
...
console.log('✅ Integrated Firebase Auth Gate to SPI Holdings successfully!');

backupClientIndex('spi_holdings', 'Integrated Strict Firebase Auth Gate');
